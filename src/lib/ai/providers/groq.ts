...
export async function generateQuizWithGroq(count: number, difficulty?: string, contextTexts?: string[]): Promise<QuizQuestion[]> {
    const apiKey = process.env.GROQ_API_KEY;
    if (!apiKey) throw new Error("GROQ_API_KEY is missing");
    ...
    try {
        ...
        console.error("Failed to parse Groq quiz response:", content);
        throw new Error("Invalid JSON format from Groq quiz");
    }
}
